...
export default class FileUploadDialogPanel extends Component {
  render = () => {
    ...
    return (
      ...
      >
        Start uploading of {this.props.selectedDicomFiles.length}{" "}
        {this.props.selectedDicomFiles.length > 1 ? "files" : "file"}.
        <Divider />
        <Steps model={states} activeIndex={this.props.uploadProcessState} />
        <Divider />
        <div hidden={!this.props.fileUploadInProgress}>
            ...
            left={
              <React.Fragment>
                <StyledDisabbledButton
                  ...
                  disabled={true}
                >
                  <Badge className="text-900" value={this.props.analysedFilesCount} />
                </StyledDisabbledButton>
                <StyledDisabbledButton
                  type="button"
                  label="Uploaded:"
                  ...
                  disabled={true}
                >
                  <Badge className="text-900" value={this.props.uploadedFilesCount} />
                </StyledDisabbledButton>
                <StyledDisabbledButton
                  type="button"
                  label="Verified:"
                  ...
                  disabled={true}
                >
                  <Badge className="text-900" value={this.props.verifiedUploadedFilesCount} />
                </StyledDisabbledButton>
              </React.Fragment>
            }
            right={
              <React.Fragment>
                <StyledCommandButton
                  ...
                  className={"text-sm p-button-success pr-3"}
                  label="Finish"
                  icon="pi pi-check"
                  iconPos="right"
                  onClick={this.props.redirectToPortal}
                />
                <StyledCommandButton
                  hidden={
                    this.props.evaluationUploadCheckResults.length == 0 && this.props.dicomUidReplacements != null
                  }
                  className={"text-sm p-button-warning  pr-3"}
                  label="Log file"
                  icon="pi pi-file"
                  iconPos="right"
                ...
                <StyledCommandButton
                  ...
                  icon="pi pi-sync"
                  iconPos="right"
                  onClick={this.props.retrySubmitUploadPackage}
                />
              </React.Fragment>
            }
          ></Toolbar>
