...
import DicomPropertyNames from "../../../src/constants/DicomPropertyNames";
import Modalities from "../../../src/constants/Modalities";
import DicomFile from "../../../src/model/DicomFile";
...
describe("Each Dicom file in the basic data path will be tested if properties are parsed according to the modality.", () => {
  ...
  test.each(files)("Testing %s file", (fileName) => {
    // ignore .md files
...
expect.extend({
  toHaveBasicDicomProperties(file) {
    ...
    if (failMessages.length === 0) {
      return {
        message: () => "Basic Dicom properties test passed",
        pass: true,
      };
    } else {
      return {
        message: () => failMessages.toString(),
        pass: false,
      };
    }
  },
...
expect.extend({
  toHaveCtSpecificDicomProperties(file) {
    ...
    if (failMessages.length === 0) {
      return {
        message: () => "CT specific Dicom properties test passed",
        pass: true,
      };
    } else {
      return {
...
function verifyModalityString(value, expectedModality) {
  ...
  if (value == null) {
    ...
    if (expectedModality != null) {
      if (value != expectedModality) failMessages.push(`${value} is not the expected modality ${expectedModality}.`);
    } else {
