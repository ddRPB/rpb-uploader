...
export default class SanityCheckResultsPanel extends Component {
  ...
  sanityCheckResultCommandsActionTemplate(node, column) {
    ...
    if (sanityCheckConfiguration[[node.category]] === true) {
      // button is active if configuration for that category is set to true
      return (
        <div>
          <StyledButton
            type="button"
  ...
  deIdentificationCheckResultCommandsActionTemplate(node, column) {
    ...
    if (deIdentificationCheckConfiguration[[node.category]] === true) {
      // button is active if configuration for that category is set to true
      return (
        <div>
          <StyledButton
   ...
   */
  renderHeader = () => {
    return <div>Sanity Check Results</div>;
  };
  ...
  render = () => {
    return (
      ...
      >
        <DataTable size="small" value={this.createCombinedRows()}>
          <Column className="text-sm" field="message" sortable header="Message" />
          <Column
            className="text-sm"
            columnKey="Disable"
            header="Commands"
            body={this.getCommandActionTemplate.bind(this)}
          />
        </DataTable>
        <ScrollTop />
      </Dialog>
