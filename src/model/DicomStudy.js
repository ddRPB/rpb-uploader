...
export default class DicomStudy {
  ...
  constructor(studyInstanceUID, studyDate, studyDescription, patientID, patientName, patientBirthDate, patientSex) {
    ...
    this.studyDescription = new Set([
      studyDescription === undefined || studyDescription === null ? "" : studyDescription,
    ]);
    this.patientID = new Set([patientID === undefined || patientID === null ? "" : patientID]);
    this.patientBirthDate = new Set([
      patientBirthDate === undefined || patientBirthDate === null ? "" : patientBirthDate,
    ]);
  ...
  getStudyType() {
    ...
    if (
      (modalities.includes("CT") || modalities.includes("MR")) &&
      modalities.includes("RTSTRUCT") &&
      !modalities.includes("RTPLAN") &&
      !modalities.includes("RTDOSE")
    ) {
      return "Contouring";
  ...
  patientPropertiesHaveDifferentValues() {
    if (this.patientID.size > 1) {
      return true;
    }
    if (this.patientBirthDate.size > 1) {
      return true;
    }
    if (this.patientSex.size > 1) {
      return true;
    }
    if (this.patientName.size > 1) {
      return true;
    }
    return false;
  }
