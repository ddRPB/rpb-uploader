...
import DicomStudyVerificationMessages, {
  ...
  GENDER_IS_NOT_VALID,
} from "../constants/DicomStudyVerificationMessages";
...
export default class DicomStudyUploadSlotVerifier {
  ...
  getVerificationResult() {
    ...
    if (this.dicomStudy.studyInstanceUID === null) {
      ...
      if (this.dicomStudy.studyInstanceUID === "")
        this.result.errors.push(DicomStudyVerificationMessages.StudyInstanceUID_IS_EMPTY);
    }
    ...
    if ("subjectSex" in this.dicomUploadSlot) {
      ...
      if (!(uploadSlotGender === "F" || uploadSlotGender === "M")) {
        this.result.errors.push(`${this.dicomUploadSlot.subjectSex} ` + GENDER_IS_NOT_VALID);
      }
      if (studyPatientSex === "") {
        this.result.errors.push(GENDER_IS_NOT_DEFINED);
      } else {
        if (uploadSlotGender != studyPatientSex) {
          this.result.errors.push(
            `Upload slot gender "${
              this.dicomUploadSlot.subjectSex
            }" is not equal to "${this.dicomStudy.getPatientSex()}"`
          );
        }
      }
    }
