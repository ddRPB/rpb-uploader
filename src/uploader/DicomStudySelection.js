...
export class DicomStudySelection extends Component {
  ...
  buildStudiesRows() {
    ...
    for (let study of Object.values(this.props.studies)) {
      ...
      study.joinedStudyDates = convertDicomDateStringArrayToLocaleString(
        study.getStudyDateArray(),
        this.props.language
      );
    ...
    if (differentPropertyValues) {
      return (
        ...
        >
          ...
          >
            <Card title="Patient Details">{patientDetailsList}</Card>
          </OverlayPanel>
        </StyledButton>
      );
    }
  ...
  createPatientDetailsList(node, key) {
    ...
    if (node.patientName != undefined) {
      if (node.patientName.size > 0) {
        patientDetailsList.push(
          <div key={key + "1"}>
            <b>Name :</b> {[...node.patientName].join(" / ")}{" "}
            {node.patientName.size === 1 ? null : this.getExclamationToolTip(inconsitentValuesMessages)}
          </div>
        );
      }
    }
    ...
    if (node.patientID != undefined) {
      if (node.patientID.size > 0) {
        patientDetailsList.push(
          <div key={key + "2"}>
            <b>PID :</b> {[...node.patientID].join(" / ")}{" "}
            {node.patientID.size === 1 ? null : this.getExclamationToolTip(inconsitentValuesMessages)}
          </div>
        );
      }
    }
    ...
    if (node.patientBirthDate != undefined) {
      if (node.patientBirthDate.size > 0) {
        patientDetailsList.push(
          <div key={key + "3"}>
            <b>Birth Date :</b>{" "}
            {convertDicomDateStringArrayToLocaleString([...node.patientBirthDate], this.props.language).join(" / ")}{" "}
            {node.patientBirthDate.size === 1 ? null : this.getExclamationToolTip(inconsitentValuesMessages)}
          </div>
        );
      }
    }
    ...
    if (node.patientSex != undefined) {
      if (node.patientSex.size > 0) {
        patientDetailsList.push(
          <div key={key + "4"}>
            <b>Sex :</b> {[...node.patientSex].join(" / ")}{" "}
            {node.patientSex.size === 1 ? null : this.getExclamationToolTip(inconsitentValuesMessages)}
          </div>
        );
      }
    }
