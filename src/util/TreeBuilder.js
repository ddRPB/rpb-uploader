            ...
            for (let seriesObject of series) {
                ...
                switch (modality) {
                    case "RTSTRUCT":
                        ...
                        struct.data = new Object();
                        struct.children = [];
                        struct.key = seriesObject.parameters.get("SeriesInstanceUID");
                    ...
                    case "RTPLAN":
                        ...
                        break;
                    case "RTDOSE":
                        const dose = new Object();
                        dose.data = new Object();
                        dose.children = [];
                    ...
                    case "CT":
                        ...
                        ct.data.details = "CT " + seriesObject.parameters.get("SOPInstanceUID");
                        ct.children = [];
                        ...
                        this.rootObjects[seriesObject.parameters.get("SOPInstanceUID")] = ct;
                        this.rootObjects[seriesObject.parameters.get("SeriesInstanceUID")] = ct;
                        this.result.root.push(ct);
                        break;
                    default:
                        this.rootObjects[seriesObject.parameters.get("SOPInstanceUID")] = seriesObject.parameters;
                }
            ...
            for (let frameOfReferenceItem of refSequence) {
                if (frameOfReferenceItem.get("RTReferencedStudySequence") !== undefined) {
                    for (let reference of frameOfReferenceItem.get("RTReferencedStudySequence")) {
                        const refSOPUID = reference.get("ReferencedSOPInstanceUID");
                        if ((this.rootObjects[refSOPUID]) !== undefined) {
                            if (this.rootObjects[refSOPUID].children !== undefined) {
                                this.rootObjects[refSOPUID].children.push(rTStruct);
                            }
                        } else {
                            for (let contourImage of reference.get("ContourImageSequence")) {
                                if ((this.rootObjects[contourImage.get("SeriesInstanceUID")]) !== undefined) {
                                    if (this.rootObjects[contourImage.get("SeriesInstanceUID")].children !== undefined) {
                                        this.rootObjects[contourImage.get("SeriesInstanceUID")].children.push(rTStruct);
                                    }
                                }
                            }
